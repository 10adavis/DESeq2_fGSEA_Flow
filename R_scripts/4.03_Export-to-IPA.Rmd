...
```{r DEG_tables}
DEG_columns<-c("Comparison","Total DEGs (adjP<0.05)","Upregulated genes (logFC>0, adjP<0.05)","Downregulated genes (logFC<0, adjP<0.05)")
# for loop
DEG_sig<-as.list(0)
DEG_row<-as.list(0)
for (i in 1:length(anno_results)){
DEG_sig[[i]]<-subset(anno_results[[i]],anno_results[[i]]$adj.P.Val<0.05)
